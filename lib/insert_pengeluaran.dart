...
class Insertkeluar extends StatefulWidget {
  final index;
  final value;
  Insertkeluar({Key key, @required this.index, @required this.value})
      : super(key: key);

  @override
  _InsertkeluarState createState() =>
      _InsertkeluarState(index: index, value: value);
}

class _InsertkeluarState extends State<Insertkeluar> {
  _InsertkeluarState({@required this.index, @required this.value}) : super();
  ...
  saveData() async {
    ...
    if (isDataValid()) {
      ...
      var savedData = await Dataluar.getData();
      ...
      await Dataluar.saveData(savedData);
